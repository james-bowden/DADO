...
class IndependentCliqueObjective3(SyntheticGraphObjective):
    """
    Implementation of independent clique objective with k=10 cliques of size=4, D=20.
    Each clique is fully connected internally but independent from other cliques.
    """
    
    def __init__(self, *args, **kwargs):
        # Set parameters
        k = 10  # number of cliques
        clique_size = 4  # size of each clique
        D = 20  # number of states per variable
        total_vars = k * clique_size  # total variables = 40
        ...
        fg = TabularGeneralizedFunctionalGraph.fixed_graph_random(
            gg, n_states_list, seed=903
        )
        
        # Initialize the parent class
        super().__init__(D=[D]*total_vars, fg=fg, *args, **kwargs)
        self.obj_name = "IndependentCliqueObjective3"
